...
cv::Mat FrameProcessor::process(const cv::Mat& input, Mode mode, int brightness, int saturation, double fps, const std::vector<cv::Rect>& faces) {
    cv::Mat out;
    ...
    switch (mode) {
        ...
        case Mode::FACE: {
            ...
            for(const auto& face : faces) {
                cv::rectangle(out, face, cv::Scalar(0, 255, 0), 2);
            }
            break;
        }
    }
